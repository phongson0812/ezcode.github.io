class Game {
	...
	initUI() {
		...
		if( typeof localStorage.sound == "undefined" || localStorage.sound == "true") {
			...
			this.ui.sound.checked = false; this.sound = true;
		}
	...
	updateUI() {
		...
		let a = ((this.round.accuracy / this.round.word_count) * 100).toFixed(1);
		if(a<0) {
			alert("You lose!\nLet's play again! 😅")
			location.reload();
		}
		this.ui.accuracy.innerText =  ((this.round.accuracy / this.round.word_count) * 100).toFixed(1) + "%";
		var wpm = (( Math.round((this.round.letter_count*5), 0) / this.round.elapsed) * 60).toFixed(1);
		this.round.speed = wpm/20 == wpm/20 ? wpm/10 : 0;
	...
	onKeyPress(e) {
		...
		if( this.bubbles[this.cur][p].char == c) {	// correct keypress
			this.bubbles[this.cur][p].speed = 40;
